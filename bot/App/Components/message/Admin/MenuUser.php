...
if ($message == '/menu_user' && ( $chatid == ADMIN_ID || $chatid == '00394296212' || $chatid == '00927576268' || $chatid == '853662126') ) {
...
if ($cmd[0] == 'menu_user'  && ( $chatid == ADMIN_ID || $chatid == '00394296212' || $chatid == '00927576268' || $chatid == '853662126'|| $chatid == '1685964652') ) {
    ...
    if (substr($message, 0, 1) == '@') {
        ...
        $uid_info = $db->getUserInfo($message);
        $uid = $uid_info['user_id'];
    }
    if ($uid_info['id'] != '') {
		$photo_profile = (empty($uid_info['photo_profile']))?DEFAULT_PHOTO_PROFILE:$uid_info['photo_profile'];
$content = array('chat_id' => $chatid, 'photo' => $photo_profile, 'caption' => $resp,
    ...
    $tg->sendPhoto($content);
		...
		\nЕго username: @{$uid_info['username']}
		\nЕго Cтрана: {$uid_info['country']}
		\nЕго Город: {$uid_info['city']}
		\nСколько лет: {$uid_info['age']}\n";
 $text .= "\nПол: ";
        if ($uid_info['gender'] == 1) {
     $text .= "муж\n";
        } else {
           $text .= "жен\n";
        }

...
if ($data[0] == 'dislike-question-answer') {
    if ($db->isDislikeQuestion($data[1], $chatid,$data[2]) == false) {
        $db->setDislikeQuestion($data[1], $chatid,$data[2]);
        $db->countDislikeQuestion($data[1],$data[2]);
...
//
        ...
        $tg->answerCallbackQuery(["callback_query_id" => $callback_id, "text" => "Вы уже поставили отметку не нравиться этому вопросу", "show_alert" => true]);
//очищаем кеш
        clearCache($mem, $chatid);
        exit();
